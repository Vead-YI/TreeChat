...
# 数据库路径
DB_PATH = Path(__file__).parent.parent.parent / "database" / "treechat.db"
    ...
    with get_db() as db:
        db.execute("""
            INSERT INTO trees (id, title, root_id, active_node_id, nodes_json, 
                             default_context_mode, tags_json, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            tree.id,
            tree.title,
...
def _row_to_tree(row: sqlite3.Row) -> ConversationTree:
    ...
    return ConversationTree(
        ...
        default_context_mode=row.get("default_context_mode", "full"),
        tags=tags,
